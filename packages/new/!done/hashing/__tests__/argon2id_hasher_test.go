...
func TestArgon2idHasher_BasicFunctionality(t *testing.T) {
	...
	hasher := manager.Hasher("argon2id")

	if hasher == nil {
		t.Fatal("Argon2id hasher should not be nil")
	}

	password := "argon2idTest123"
...
func TestArgon2idHasher_WithConfiguration(t *testing.T) {
	// Create mock container with config
	container := containerMocks.NewMockContainer()
	config := configMocks.NewMockConfig()

	// Set argon2id configuration
	...
	if info.AlgoName != "argon2id" {
		t.Errorf("Algorithm should be argon2id, got %s", info.AlgoName)
	}
...
func TestArgon2idHasher_WithOverrideOptions(t *testing.T) {
	// Create mock container with config
	container := containerMocks.NewMockContainer()
	config := configMocks.NewMockConfig()

	// Set default argon2id configuration
	config.Set("hashing.argon2id", map[string]interface{}{
		"memory":  65536,
		"time":    1,
		"threads": uint8(1),
...
func TestArgon2idHasher_InvalidOptions(t *testing.T) {
	// Create mock container with config
	container := containerMocks.NewMockContainer()
	config := configMocks.NewMockConfig()
	...
	if err == nil {
		t.Error("Expected error for invalid argon2id memory")
	}
	...
	if err == nil {
		t.Error("Expected error for invalid argon2id threads")
	}
	...
	if err == nil {
		t.Error("Expected error for invalid argon2id time")
	}
}
...
func TestArgon2idHasher_LongPassword(t *testing.T) {
	// Create mock container with config
	container := containerMocks.NewMockContainer()
	config := configMocks.NewMockConfig()
	...
	if err != nil {
		...
		if !strings.Contains(err.Error(), "too long") {
			t.Errorf("Expected 'too long' error for argon2id, got: %v", err)
		}
	}
	// Note: Argon2id might handle long passwords fine, so err == nil is also valid
}
